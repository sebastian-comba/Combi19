                ...
                $(form).ajaxSubmit((data) => {
                    ...
                    if (data.viajes.length) {
                        ...
                        for (let i = 0; i < data.viajes.length; i++) {
                            ...
                            viajes.innerHTML = viajes.innerHTML + ' <div class="card"> <div div class="card-title" style = "background-color:#0a546b; color: azure; text-align: center;" > <strong>Tipo:</strong> ' + e.combi.tipo + '</div ><div class="card-body"><div class="row"><div class="col-6"><p><strong>Origen:</strong> ' + e.ruta.origen.nombre + '</p><p><strong>Destino:</strong>' + e.ruta.destino.nombre + '</p><p><strong>Fecha y hora:</strong> ' + (new Date(Date.parse(e.fecha))).completa() + '</div><div class="col-6"> <p><strong>Cantidad de ascientos:</strong> ' + e.asientosDisponibles + '</p><br><p><strong>Precio:</strong> $' + e.precio +'</p></div></div></p><form action="" method="get" ><button>Comprar</button> </form></div></div > <br> ';

                        }
                        
                    } else {
                        viajes.innerHTML = '<p>No hay viajes disponibles con esas especificaciones por favor vuelva a aintentarlo</p>'
                    }
                });
            }
        });
